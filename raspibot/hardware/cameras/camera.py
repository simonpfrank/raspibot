...
class Camera:
    ...
    def __init__(self, 
        ...
        if not PICAMERA2_AVAILABLE:
            raise ImportError("picamera2 not available. Camera requires picamera2 library.")
        
        self.logger = setup_logging(__name__)
        
        # Load defaults from config
        self._load_config_defaults(camera_device_id, camera_resolution, display_resolution, 
                                 display_position, display_mode)
        ...
        self.logger.info(f"Camera initialized: {self.camera_type} camera {self.camera_device_id}")
    
    def _load_config_defaults(self, camera_device_id, camera_resolution, display_resolution, 
                            display_position, display_mode) -> None:
    ...
    def _detect_camera_type(self) -> str:
        ...
        for info in camera_info:
            ...
            if "imx500" in model:
                if first_pi_ai is None:
                    first_pi_ai = info
            elif ("imx" in model or "pi" in model) and "uvc" not in model and "usb" not in camera_id:
                if first_pi is None:
                    first_pi = info
        ...
        self.logger.info(f"Detected {camera_type} camera: {selected_camera.get('Model', 'Unknown')}")
        return camera_type
    ...
    def _initialize_hardware(self) -> None:
        ...
        try:
            ...
            self.logger.info(f"{self.camera_type.title()} Camera hardware initialized successfully")
        except Exception as e:
            self.logger.error(f"Camera._initialize_hardware failed: {type(e).__name__}: {e}")
            raise
    ...
    def _setup_ai_detection(self) -> None:
        ...
        try:
            ...
            self.logger.error(f"Camera._setup_ai_detection failed: {type(e).__name__}: {e}")
            raise
    ...
    def _process_ai_detections(self, tracked_objects):
        ...
        try:
            ...
            raw_detections = self._convert_detection_to_dict(boxes, scores, classes, metadata)
            if not raw_detections:
                return
            
            # Filter by confidence threshold
            confidence_filtered = [d for d in raw_detections if d['score'] > self.confidence_threshold]
            if not confidence_filtered:
                return
            
            # Apply NMS and update tracking
            self.detections = self._apply_nms(confidence_filtered, iou_threshold=NMS_IOU_THRESHOLD)
    ...
    def _convert_detection_to_dict(self, boxes, scores, classes, metadata) -> List[Dict[str, Any]]:
    ...
    def _filter_valid_boxes(self, detections: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for label, group_detections in label_groups.items():
            ...
            while sorted_detections:
                ...
                for detection in sorted_detections:
                    if self._calculate_iou(current['box'], detection['box']) < iou_threshold:
    ...
    def _associate_detections_to_tracks(self, detections: List[Dict[str, Any]], 
        ...
        for detection in detections:
            ...
            for tracked_object in tracked_objects:
                if (not tracked_object.get('last_detection') or
                    tracked_object['label'] != detection['label'] or
                    tracked_object['seen_this_frame']):
                    continue
                
                iou = self._calculate_iou(detection['box'], tracked_object['last_detection']['box'])
...
if __name__ == "__main__":
    try:
        ...
        # Start the camera
        if camera.start():
            print(f"{camera.camera_type.title()} camera started successfully. Close the preview window to stop.")
