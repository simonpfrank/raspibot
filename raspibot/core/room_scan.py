...
class RoomScanner:
    ...
    def __init__(
        ...
        # Hardware
        self.camera = camera or Camera(face_detection=True)
    ...
    def scan_room(self) -> List[Dict]:
        ...
        self.logger.info(
            f"Scanning {len(self.scan_positions)} positions: {[f'{p:.1f}°' for p in self.scan_positions]}"
        )
        ...
        try:
            for i, pan_angle in enumerate(self.scan_positions):
                self.logger.info(
                    f"[{i+1}/{len(self.scan_positions)}] Scanning position: Pan={pan_angle:.1f}°"
                )

                # Move to position
                self.scan_pattern.move_to_position(
                    self.servo_controller, pan_angle, self.scan_tilt
                )
                self.logger.debug(
                    f"Moved to Pan={pan_angle:.1f}°, Tilt={self.scan_tilt:.1f}°"
                )
                ...
                position_detections = self._capture_detections_at_position(
                    pan_angle, self.scan_tilt, i
                )
                all_detections.extend(position_detections)

                self.logger.info(
                    f"Position {i+1} captured {len(position_detections)} detections"
                )

            # Return to center
            self.logger.info("Returning to center position")
            self.scan_pattern.move_to_position(
                self.servo_controller, SERVO_PAN_CENTER, SERVO_TILT_CENTER
            )
        ...
        self.logger.info(
            f"Room scan complete: {len(unique_objects)} unique objects found"
        )
    ...
    async def scan_room_async(self) -> List[Dict]:
        ...
        try:
            for i, pan_angle in enumerate(self.scan_positions):
                ...
                await self.scan_pattern.move_to_position_async(
                    self.servo_controller, pan_angle, self.scan_tilt
                )
                self.logger.debug(
                    f"Moved to Pan={pan_angle:.1f}°, Tilt={self.scan_tilt:.1f}°"
                ...
                # Capture detections at this position
                position_detections = await self._capture_detections_at_position_async(
                    pan_angle, self.scan_tilt, i
                )
                all_detections.extend(position_detections)
        ...
        self.last_scan_data = unique_objects
        self.logger.info(
            f"Async room scan complete: {len(unique_objects)} unique objects found"
        )
    ...
    def get_scan_summary(self) -> Dict:
        ...
        summary = {
            ...
            "objects_by_type": {
                label: len(objects) for label, objects in by_label.items()
            },
    ...
    def _capture_detections_at_position(
        ...
        self.logger.debug(
            f"Capturing {self.frames_per_position} frames at position {position_index}"
        )
            ...
            if tracked_objects:
                for obj in tracked_objects:
                    ...
                    if (
                        ...
                        world_angle = self.deduplicator._calculate_world_angle(
                            box, pan_angle
                        )
        ...
        self.logger.debug(
            f"Position {position_index} captured {len(detections)} raw detections"
        )
        return detections

    async def _capture_detections_at_position_async(
        self, pan_angle: float, tilt_angle: float, position_index: int
    ) -> List[Dict]:
        ...
        for frame_num in range(self.frames_per_position):
            # Get current tracked objects
            tracked_objects = await asyncio.to_thread(
                lambda: self.camera.tracked_objects
            )
...
if __name__ == "__main__":
    ...
    # Create scanner
    scanner = RoomScanner(
        frames_per_position=10, settling_time=1.0, confidence_threshold=0.6
    )

    print("=== Room Scanner Demo ===")
    print(
        f"Scan positions: {[f'{p:.1f}°' for p in scanner.scan_pattern.calculate_positions()]}"
    )
    ...
    try:
        ...
        for i, obj in enumerate(objects, 1):
            print(
                f"  {i}. {obj['label']}: {obj['confidence']:.2f} confidence at {obj['world_angle']:.1f}°"
            )
